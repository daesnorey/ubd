...
CREATE MATERIALIZED VIEW CLIENT_HISTORY_MV
REFRESH COMPLETE NEXT (ROUND(SYSDATE) + 1/24) + 7 AS
SELECT  PRODUCTO.NOMBRE AS PRODUCT_NAME, SUM(1) AS AMOUNT, TO_CHAR(FACTURA.FECHA_ALTA, 'Month') as MONTH, TO_CHAR(FACTURA.FECHA_ALTA, 'yyyy') AS YEAR,
        TERCERO.ID_TIPO_DOCUMENTO, TERCERO.NUMERO_DOCUMENTO, TERCERO.NOMBRES, TERCERO.APELLIDOS
FROM    FACTURA INNER JOIN
        CLIENTE ON CLIENTE.ID_CLIENTE = FACTURA.ID_CLIENTE INNER JOIN
        TERCERO ON TERCERO.ID_TERCERO = CLIENTE.ID_TERCERO INNER JOIN
        DETALLE_FACTURA ON DETALLE_FACTURA.ID_FACTURA = FACTURA.ID_FACTURA INNER JOIN
        INVENTARIO ON INVENTARIO.ID_INVENTARIO = DETALLE_FACTURA.ID_INVENTARIO INNER JOIN
        PRODUCTO ON PRODUCTO.ID_PRODUCTO = INVENTARIO.ID_PRODUCTO
WHERE   FACTURA.FECHA_ALTA >= sysdate - interval '2' year
GROUP BY
        PRODUCTO.NOMBRE, TERCERO.ID_TERCERO, TO_CHAR(FACTURA.FECHA_ALTA, 'Month'), TO_CHAR(FACTURA.FECHA_ALTA, 'yyyy'),
        TERCERO.ID_TIPO_DOCUMENTO, TERCERO.NUMERO_DOCUMENTO, TERCERO.NOMBRES, TERCERO.APELLIDOS;

CREATE MATERIALIZED VIEW SELL_PRODUCT_HISTORY_MV
REFRESH COMPLETE NEXT (ROUND(SYSDATE) + 1/24) + 7 AS
SELECT  PRODUCTO.NOMBRE AS PRODUCT_NAME, SUM(1) AS AMOUNT, TO_CHAR(FACTURA.FECHA_ALTA, 'Month') as MONTH, TO_CHAR(FACTURA.FECHA_ALTA, 'yyyy') AS YEAR
FROM    FACTURA INNER JOIN
        DETALLE_FACTURA ON DETALLE_FACTURA.ID_FACTURA = FACTURA.ID_FACTURA INNER JOIN
        INVENTARIO ON INVENTARIO.ID_INVENTARIO = DETALLE_FACTURA.ID_INVENTARIO INNER JOIN
        PRODUCTO ON PRODUCTO.ID_PRODUCTO = INVENTARIO.ID_PRODUCTO
WHERE   FACTURA.FECHA_ALTA >= sysdate - interval '5' year
GROUP BY
        PRODUCTO.NOMBRE, TO_CHAR(FACTURA.FECHA_ALTA, 'Month'), TO_CHAR(FACTURA.FECHA_ALTA, 'yyyy');

CREATE MATERIALIZED VIEW PRODUCTION_EMPLOYEE_HISTORY_MV
REFRESH COMPLETE NEXT (ROUND(SYSDATE) + 1/24) + 30 AS
SELECT  PRODUCTO.NOMBRE AS PRODUCT_NAME, SUM(1) AS AMOUNT, TO_CHAR(PRODUCCION.FECHA_ALTA, 'Month') as MONTH, TIPO_PRODUCTO.NOMBRE AS PRODUCT_LINE,
        TERCERO.ID_TIPO_DOCUMENTO, TERCERO.NUMERO_DOCUMENTO, TERCERO.NOMBRES, TERCERO.APELLIDOS, PRESENTACION.NOMBRE AS PRESENTATION
FROM    PRODUCCION INNER JOIN
        PROCESO_PRODUCCION ON PROCESO_PRODUCCION.ID_PRODUCCION = PRODUCCION.ID_PRODUCCION INNER JOIN
        PRODUCTO ON PRODUCTO.ID_PRODUCTO = PROCESO_PRODUCCION.ID_PRODUCTO INNER JOIN
        TIPO_PRODUCTO ON TIPO_PRODUCTO.ID_TIPO_PRODUCTO = PRODUCTO.ID_TIPO_PRODUCTO INNER JOIN
        PRESENTACION ON PRESENTACION.ID_PRESENTACION = PROCESO_PRODUCCION.ID_PRESENTACION INNER JOIN
        EMPLEADO ON EMPLEADO.ID_EMPLEADO = PROCESO_PRODUCCION.ID_EMPLEADO INNER JOIN
        TERCERO ON TERCERO.ID_TERCERO = EMPLEADO.ID_TERCERO
WHERE   PRODUCCION.FECHA_ALTA >= sysdate - interval '12' month AND EMPLEADO.FECHA_RETIRO IS NULL
GROUP BY
        PRODUCTO.NOMBRE, TERCERO.ID_TERCERO, TO_CHAR(PRODUCCION.FECHA_ALTA, 'Month'), TIPO_PRODUCTO.NOMBRE,
        TERCERO.ID_TIPO_DOCUMENTO, TERCERO.NUMERO_DOCUMENTO, TERCERO.NOMBRES, TERCERO.APELLIDOS, PRESENTACION.NOMBRE;

CREATE MATERIALIZED VIEW COST_HISTORY_MV
REFRESH COMPLETE NEXT (ROUND(SYSDATE) + 1/24) + 180 AS
SELECT  DETALLE_COMPRA.ID_PRODUCTO, SUM(DETALLE_COMPRA.VALOR_TOTAL) PRECIO_COMPRA, TO_CHAR(COMPRA.FECHA, 'Month') AS MONTH,
        TO_CHAR(COMPRA.FECHA, 'yyyy') AS YEAR, SUM(DETALLE_FACTURA.VALOR) AS PRECIO_VENTA
FROM    DETALLE_COMPRA INNER JOIN
        COMPRA ON COMPRA.ID_COMPRA = DETALLE_COMPRA.ID_COMPRA INNER JOIN
        INVENTARIO ON INVENTARIO.ID_PRODUCTO = DETALLE_COMPRA.ID_PRODUCTO AND TO_CHAR(INVENTARIO.FECHA_ALTA, 'yyyy') = TO_CHAR(COMPRA.FECHA, 'yyyy') AND TO_CHAR(INVENTARIO.FECHA_ALTA, 'Month') = TO_CHAR(COMPRA.FECHA, 'Month') INNER JOIN
        DETALLE_FACTURA ON DETALLE_FACTURA.ID_INVENTARIO = INVENTARIO.ID_INVENTARIO
WHERE   COMPRA.FECHA >= sysdate - interval '5' year
GROUP BY
        DETALLE_COMPRA.ID_PRODUCTO, TO_CHAR(COMPRA.FECHA, 'Month'), TO_CHAR(COMPRA.FECHA, 'yyyy')
ORDER BY
        TO_CHAR(COMPRA.FECHA, 'Month'), TO_CHAR(COMPRA.FECHA, 'yyyy');


select table_name, constraint_name, status, owner
from all_constraints
where r_owner = 'PRE_DNOVOA'
